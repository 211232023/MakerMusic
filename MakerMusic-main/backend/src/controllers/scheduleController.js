...
exports.createSchedule = async (req, res) => {
  ...
  try {
    // Adicionando o campo 'activity' na query de inserção
    const [result] = await pool.query(
      'INSERT INTO schedules (student_id, teacher_id, day_of_week, start_time, end_time, activity) VALUES (?, ?, ?, ?, ?, ?)',
      [studentId, teacherId, dayOfWeek, startTime, endTime, activity]
    );
    res.status(201).json({ message: 'Horário criado com sucesso!', scheduleId: result.insertId });
  } catch (error) {
    console.error('Erro ao criar horário:', error);
    // Se o erro for por falta da coluna, o log mostrará 'Unknown column 'activity' in 'field list''
    res.status(500).json({ message: 'Erro no servidor ao salvar o horário. Verifique se a coluna "activity" existe na tabela "schedules".' });
  }
};
...
exports.getSchedulesByStudent = async (req, res) => {
    ...
    try {
        const [schedules] = await pool.query(
            `SELECT s.id, s.day_of_week, s.start_time, s.end_time, s.activity, u.name as teacher_name
             FROM schedules s
